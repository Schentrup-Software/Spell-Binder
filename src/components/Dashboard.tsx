...
export default function Dashboard() {
  ...
  const calculateStats = async () => {
    try {
      ...
      const totalCards = collectionEntries.reduce((sum, entry) => sum + entry.quantity, 0)
      
      // Count unique cards
      const uniqueCards = collectionEntries.length
      
      // Count unique sets
      const uniqueSets = new Set(collectionEntries.map(entry => entry.card?.set_code).filter(Boolean))
      const setsRepresented = uniqueSets.size
      
      // Calculate total value
      const totalValue = collectionEntries.reduce((sum, entry) => {
        const cardPrice = entry.card?.price_usd || 0
        return sum + (cardPrice * entry.quantity)
      }, 0)
      ...
      collectionEntries.forEach(entry => {
        ...
        if (!entry.card.colors || entry.card.colors.length === 0) {
          colorDistribution['C'] = (colorDistribution['C'] || 0) + entry.quantity
          return
        }
        
        // Handle multicolor cards
        if (entry.card.colors.length > 1) {
          colorDistribution['M'] = (colorDistribution['M'] || 0) + entry.quantity
          return
        }
        
        // Handle single color cards
        const color = entry.card.colors[0]
      ...
      collectionEntries.forEach(entry => {
        ...
        typeDistribution[primaryType] = (typeDistribution[primaryType] || 0) + entry.quantity
      })
                    ...
                    <div key={entry.id} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex items-start space-x-3">
                        <div className="flex-shrink-0">
                          <CardImage 
                            card={entry.card}
                            cardName={entry.card?.name || 'Unknown Card'} 
                            size="small"
                            quality="low"
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {entry.card?.set_name || 'Unknown'} ({entry.card?.set_code || ''})
                          </div>
                          <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
                            <span>Qty: {entry.quantity}</span>
                            <span>{entry.condition} {entry.foil ? '(Foil)' : ''}</span>
                            <span>{formatDate(entry.acquired_date)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {stats.recentlyAdded.map(entry => (
                        <tr key={entry.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-900">{entry.card?.set_name || 'Unknown'}</div>
                            <div className="text-xs text-gray-500">{entry.card?.set_code || ''}</div>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {entry.quantity}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {entry.condition} {entry.foil ? '(Foil)' : ''}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {formatDate(entry.acquired_date)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
